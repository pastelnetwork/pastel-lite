    ...
    */
    ...
    bool verify(std::string &error, const unsigned char* msg, const uint64_t nMessageLength)
    {
        ...
        do
        {
            ...
            if (!bRet)
                error = fmt::format("Failed to verify signature. {0}", error);
        } while (false);
        return bRet;
    }
...
#undef IS_LEFT_SIBLING
    ...
    {
        int nBotanCode = 0; // Use 0 for failure, 1 for success
        try
        {
            std::string shake_name = fmt::format("SHAKE-128({})", nOutputLength * 8);
            std::unique_ptr<Botan::HashFunction> hash(Botan::HashFunction::create(shake_name));
            if (!hash)
            {
                return 0; // Hash creation failed
            }

            hash->update(data, nDataLength);

            // Using Botan::secure_vector to store the XOF output
            Botan::secure_vector<uint8_t> output = hash->final();
            // Ensuring the requested output length does not exceed the buffer size
            if (output.size() > nOutputLength)
            {
                output.resize(nOutputLength);
            }

            std::memcpy(out, output.data(), output.size());
            nBotanCode = 1; // Success
        }
        catch (const Botan::Exception& e)
        {
            nBotanCode = 0; // Exception caught, failure
        }
        return nBotanCode;
    }
