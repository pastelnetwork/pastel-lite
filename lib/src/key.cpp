...
bool CKey::VerifyPubKey(const CPubKey& pubkey) const {
    ...
    unsigned char rnd[8];
    string str = "Pastel key verification\n";
...
bool CKey::Derive(CKey& keyChild, ChainCode &ccChild, const unsigned int nChild, const ChainCode& cc) const
{
    ...
    secp256k1_context* secp256k1_context_sign = ECCContext::getSignContext();
    const bool ret = secp256k1_ec_seckey_tweak_add(secp256k1_context_sign, (unsigned char*)keyChild.cbegin(), vout.data());
    keyChild.fCompressed = true;
    keyChild.fValid = ret;
    return ret;
}

std::optional<CExtKey> CExtKey::Derive(unsigned int numChild) const
{
    CExtKey out;
    out.nDepth = nDepth + 1;
    ...
    out.nChild = numChild;
    out.nChild = numChild;
    if (key.Derive(out.key, out.chaincode, numChild, chaincode)) {
        return out;
    } else {
        return std::nullopt;
    }
