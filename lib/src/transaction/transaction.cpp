...
string COutPoint::ToString() const {
    return fmt::format("COutPoint({}, {})", hash.ToString().substr(0, 10), n == -1 ? 0 : n);
}

string COutPoint::ToStringShort() const {
    return fmt::format("{}-{}", hash.ToString().substr(0, 64), n == -1 ? 0 : n);
}

//TODO: do we want to support Shielded stuff in lite wallet??
...
string CTxOut::ToString() const {
    return fmt::format("CTxOut(nValue={}.{:08d}, scriptPubKey={})", nValue / COIN, nValue % COIN,
...
string CMutableTransaction::ToString() const {
    string str;
    str += fmt::format("CMutableTransaction(hash={}, ver={}, vin.size={}, vout.size={}, nLockTime={})\n",
...
string CTransaction::ToString() const {
    string str;
    if (!fOverwintered) {
        str += fmt::format("CTransaction(hash={}, ver={}, vin.size={}, vout.size={}, nLockTime={})\n",
                           GetHash().ToString().substr(0, 10),
                           nVersion,
                           vin.size(),
                           vout.size(),
        ...
        str += fmt::format("CTransaction(hash={}, ver={}, fOverwintered={}, nVersionGroupId={:08x}, vin.size={}, "
                           "vout.size={}, nLockTime={}, nExpiryHeight={}, valueBalance={}"
                           //                           ", vShieldedSpend.size={}, vShieldedOutput.size={}"
                           ")\n",
                           GetHash().ToString().substr(0, 10),
                           nVersion,
                           fOverwintered,
...
//                         vShieldedOutput.size());
        ...
        str += fmt::format(
                "CTransaction(hash={}, ver={}, fOverwintered={}, nVersionGroupId={:08x}, vin.size={}, vout.size={}, nLockTime={}, nExpiryHeight={})\n",
                GetHash().ToString().substr(0, 10),
                nVersion,
                fOverwintered,
                nVersionGroupId,
