...
#include "pubkey.h"
#include "hash.h"
#include "transaction/interpreter.h"
...
uint256 SignatureHash(const CScript& scriptCode, const CTransaction& txTo, unsigned int nIn, const uint8_t nHashType, const CAmount amount)
{
...
bool MutableTransactionSignatureChecker::VerifySignature(
    const v_uint8& vchSig, const CPubKey& pubkey, const uint256& sighash) const
{
    return pubkey.Verify(sighash, vchSig);
}

bool MutableTransactionSignatureChecker::CheckSig(
    ...
    const CScript& scriptCode) const
{
    ...
    uint256 sighash;
    try {
        sighash = SignatureHash(scriptCode, txToCheck, nIn, nHashType, amount);
...
bool MutableTransactionSignatureChecker::CheckLockTime(const CScriptNum& nLockTime) const
{
    ...
    if (!(
            (txToCheck.nLockTime < LOCKTIME_THRESHOLD && nLockTime < LOCKTIME_THRESHOLD) ||
            (txToCheck.nLockTime >= LOCKTIME_THRESHOLD && nLockTime >= LOCKTIME_THRESHOLD)
    ))
        return false;
    ...
    if (nLockTime > (int64_t)txToCheck.nLockTime)
        return false;
