...
string base_blob<BITS>::GetHex() const {
    ...
    for (unsigned int i = 0; i < sizeof(data); i++) {
        ss << std::hex << std::setfill('0') << std::setw(2)
           << static_cast<unsigned int>(data[sizeof(data) - i - 1]);
    }
    return ss.str();
}
...
bool parse_uint256(string& error, uint256& value, const string &sUint256, const char *szValueDesc)
{
    ...
    do
    {
        ...
        if (sUint256.size() != uint256::SIZE * 2)
        {
            ...
            << " value size: " << sUint256.size() << ", expected: " << (uint256::SIZE * 2) << ". [" << sUint256 << "]";
            error = ss.str();
        ...
        {
            // Apple Clang (15.0.0) still doesn't support std::format
            //error = format("Invalid {} hexadecimal value: {}",
            //               szValueDesc ? szValueDesc : "uint256", sUint256);
            std::stringstream ss;
            ss << "Invalid " << (szValueDesc ? szValueDesc : "uint256")
            << " hexadecimal value: " << sUint256;
            error = ss.str();
            break;
        }
