...
constexpr auto ERRMSG_INVALID_STREAM_POS = "Invalid starting stream position {}, stream size = {}";
    ...
    const bool bUncompressData)
{
    ...
    do
    {
        ...
        {
            error = fmt::format(ERRMSG_INVALID_STREAM_POS, nStreamPos, vch.size());
            break;
        }
        nReadPos = nStreamPos;
        if (bCompressed)
...
bool CCompressedDataStream::CompressData(std::string& error, const size_t nKeepUncompressedSize, fnUncompressedDataHandler handler)
{
    ...
    do
    {
        ...
        {
            error = fmt::format(ERRMSG_INVALID_STREAM_POS, nKeepUncompressedSize, vch.size());
            break;
        }
...
bool CCompressedDataStream::Decompress(string &error)
{
    ...
    do
    {
        ...
        {
            error = fmt::format("invalid compressor data size {}", nCompressorDataSize);
            break;
        }
